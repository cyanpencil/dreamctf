...
SINV = [0 for _ in range(256)]
for i, s in enumerate(SBOX):
    SINV[s] = i
def invS(b):
    hexes = list(b.hex())
    for i in range(2*BLOCKLEN - 2, -1, -1):
        n = int(hexes[i] + hexes[i + 1], 16)
        n = SINV[n]
        hexes[i:i+2] = list(hex(n)[2:].zfill(2))
    return bytes.fromhex(''.join(hexes))

assert invS(S(b"A" * BLOCKLEN)) == b"A" * BLOCKLEN

start = lambda: remote(args.HOST, args.PORT) if not args.LOCAL else process(["python", "indigestion.py"])

io = start()
io.recvuntil("... ")
challenge = bytes.fromhex(io.recvline().strip().decode()) + b"*" * 12
h1 = H(challenge)
h2 = b"ictf{_solution_}"
m = xor(invS(h2), h1)
io.sendline((challenge + m).hex())
io.stream()
